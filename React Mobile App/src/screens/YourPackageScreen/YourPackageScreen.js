...
import { ViewPackageById } from '../../components/ViewPackageById';

export const YourPackageScreen = ({ route, navigation }) => {
  const size = 70;
  const color = '#6c63ff';
  const [packageState, setPackageState] = useState(route.params.state);
  const [customerPackageState, setCustomerPackageState] = useState(
    route.params.packageCode
  );

  if (route.params.entity === 'customer')
    console.log(
      ViewPackageById(route.params.packageCode, setCustomerPackageState)
    );

  if (customerPackageState === 'error') {
    ToastAndroid.showWithGravity(
      'No package found with this Id',
      ToastAndroid.SHORT,
      ToastAndroid.CENTER
    );
    setTimeout(() => {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
      <View style={styles.container}>
        <View style={styles.depositRow}>
          ...
          {packageState || customerPackageState === 'pending' ? (
            <EntypoIcon
        ...
        <View style={styles.depositRow}>
          ...
          {packageState || customerPackageState === 'delivering' ? (
            <EntypoIcon
              name="arrow-long-left"
              style={styles.arrow}
            ></EntypoIcon>
        ...
        <View style={styles.depositRow}>
          ...
          {packageState || customerPackageState === 'done' ? (
            <EntypoIcon
              name="arrow-long-left"
              style={styles.arrow}
            ></EntypoIcon>
